...
/**
 * Simple client for writing one shot metrics to graphite. No socket reuse or
...
public class SimpleGraphiteClient extends BaseClient {
	 ...
	 * @param graphitePort
	 *            Graphite socket. Default is 2003
	 */
	...
	public void sendMetrics(Map<String, Integer> metrics, long timeStamp) {
		try {
			...
			for (Map.Entry<String, Integer> metric : metrics.entrySet()) {
				out.printf("%s %d %d%n", metric.getKey(), metric.getValue(),
						timeStamp);
			}
			out.close();
			socket.close();
	...
	@Override
	public void sendMetric(String key, int value, long timeStamp, int count) {
